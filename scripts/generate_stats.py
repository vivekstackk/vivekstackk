...
def mock_summary():
    ...
    while cur_date <= today:
        ...
        for _ in range(7):
            ...
            count = 2 if cur_date.weekday() < 5 and (cur_date.day % 2 == 0) else (1 if cur_date.day % 5 == 0 else 0)
            week_days.append({"contributionCount": count, "date": cur_date.isoformat(), "weekday": wd})
            cur_date += timedelta(days=1)
        weeks.append(week_days)
    days = [d for w in weeks for d in w]
    weekly = [sum(d["contributionCount"] for d in w) for w in weeks]
    cur = dict(length=3, start=(today - timedelta(days=2)).isoformat(), end=today.isoformat())
    best = dict(length=12, start=(today - timedelta(days=50)).isoformat(), end=(today - timedelta(days=39)).isoformat())
    by_size = [("TypeScript", 280000), ("JavaScript", 160000), ("C++", 95000), ("HTML", 45000), ("CSS", 35000)]
    by_repo = [("TypeScript", 4), ("JavaScript", 2), ("C++", 2), ("Java", 1), ("HTML", 1)]
    return dict(
        total=sum(weekly), active=sum(1 for d in days if d["contributionCount"] > 0),
        best_week=max(weekly) if weekly else 0,
        weekly=weekly, weeks=weeks, current=cur, longest=best,
        by_size=by_size, by_repo=by_repo)


def main():
    token = os.environ.get("GITHUB_TOKEN")
    login = os.environ.get("GH_LOGIN", "vivekstackk")
    out_dir = os.environ.get("OUT_DIR", ".")

    if token:
        s = summarise(fetch(login, token))
    else:
        print("Note: GITHUB_TOKEN not set locally; generating initial baseline stats. GitHub Actions will refresh live data on schedule.")
        s = mock_summary()
