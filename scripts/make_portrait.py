...
def prep(path, crop=None, curve=CURVE, skip_rembg=False):
    """Cut out the background, even the local contrast, then darken."""
    src = Image.open(path).convert("RGBA")
    if crop:
        src = src.crop(crop)

    if (not skip_rembg) and (remove is not None):
        try:
            cut = remove(src)
        except Exception:
            cut = src
...
def main():
    ...
    ap.add_argument("--curve", type=float, default=CURVE, help="darkening curve")
    ap.add_argument("--skip-rembg", action="store_true", help="skip rembg model download/removal")
    ...
    lines = to_lines(prep(args.photo, crop, curve=args.curve, skip_rembg=args.skip_rembg), cols=args.cols)
    if args.preview:
        print("\n".join(lines))
